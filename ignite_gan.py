...
if __name__ == "__main__":
	...
	true_labels_v = T.ones(BATCH_SIZE, device=device)
	fake_labels_v = T.zeros(BATCH_SIZE, device=device)

	def process_batch(trainer, batch):
		gen_input_v = T.FloatTensor(BATCH_SIZE, LATENT_VECTOR_SIZE, 1, 1)
	...
	def log_losses(trainer):
		if trainer.state.iteration % REPORT_EVERY_ITER == 0:
			log.info(
				"%d: gen_loss=%f, dis_loss=%f", trainer.state.iteration,
